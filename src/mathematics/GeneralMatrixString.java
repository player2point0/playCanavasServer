...
//		return is;
    ...
    public void setCapacity(int mincap)
    {
    	if(value == null)
    	{
    		value = new String[mincap];
        ...
        String[] olddata = value;
        value = new String[mincap];
        System.arraycopy(olddata,0,value,0,Math.min(olddata.length,mincap));    		
    }
    ...
    public void push_back_rows(String[] rows)
    {
    	appendRows(rows.length);
    	System.arraycopy(rows, 0, value, width*(height-rows.length), width*rows.length);
    }
